...
<head>
	<meta chraset="utf-8">
	<title>James Stephenson CWB 150 - Assignment 3: Add Patron</title>
...
<body>
	...
	<?php
		...
		if ($validationFlag == 'Y')
		{
			...
			//***********************************
			//Write user info to patrons.txt file
			//(should only be added to patrons.txt if info is validated)
			//***********************************
			//saving file location to a variable
			$fileName = 'data/patrons.txt';
			
			$filePointer = fopen($fileName, 'a');  //open file in append mode
			
			//create output line by concatenating variable strings with |
			$outputLine = $lastName.'|'.$firstName.'|'.$email.'|'.$birthYear.'|'.$cityName.'|'."\n";
			
			//use fwrite() to add the output line to patrons.txt
			fwrite($filePointer, $outputLine);
		...
		print "<p>For Admin Use Only: <a href='assignment_3_view_patrons.php'>";
		print "<span style='text-decoration: underline; color: blue;'>View Patrons</span></a></p>";
